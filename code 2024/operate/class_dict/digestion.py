...
def ingest_Header(header):
    class data:
        m_packetFormat = header.m_packetFormat
...
def ingest_sessiondata(Packet):
    ...
    class data:
        ...
        m_recoveryMode = Packet.m_recoveryMode
        m_flashbackLimit = Packet.m_flashbackLimit
        m_surfaceType = Packet.m_surfaceType
        m_lowFuelMode = Packet.m_lowFuelMode
        m_raceStarts = Packet.m_raceStarts
        m_tyreTemperature = Packet.m_tyreTemperature
        m_pitLaneTyreSim = Packet.m_pitLaneTyreSim
        m_carDamage = Packet.m_carDamage
        m_carDamageRate = Packet.m_carDamageRate
        m_collisions = Packet.m_collisions
        m_collisionsOffForFirstLapOnly = Packet.m_collisionsOffForFirstLapOnly
        m_mpUnsafePitRelease = Packet.m_mpUnsafePitRelease
        m_mpOffForGriefing = Packet.m_mpOffForGriefing
        m_cornerCuttingStringency = Packet.m_cornerCuttingStringency
        m_parcFermeRules = Packet.m_parcFermeRules
        m_pitStopExperience = Packet.m_pitStopExperience
        m_safetyCar = Packet.m_safetyCar
        m_safetyCarExperience = Packet.m_safetyCarExperience
        m_formationLap = Packet.m_formationLap
        m_formationLapExperience = Packet.m_formationLapExperience
        m_redFlags = Packet.m_redFlags
        m_affectsLicenceLevelSolo = Packet.m_affectsLicenceLevelSolo
        m_affectsLicenceLevelMP = Packet.m_affectsLicenceLevelMP
        m_weekendStructure = expand(Packet.m_weekendStructure)
        m_numSessionsInWeekend = Packet.m_numSessionsInWeekend
        m_sector2LapDistanceStart = Packet.m_sector2LapDistanceStart
        m_sector3LapDistanceStart = Packet.m_sector3LapDistanceStart

    return data

def ingest_lapdata(Packet):
    m_header = ingest_Header(Packet.m_header)
    lapData=expand(Packet.m_lapData)
    allLapData=[]
    for x in lapData:
        class lap:
            m_lastLapTimeInMs = x.m_lastLapTimeInMs/1000
            ...
            m_deltaToCarInFrontMSPart = x.m_deltaToCarInFrontMSPart/1000
            m_deltaToCarInFrontMinutesPart = x.m_deltaToCarInFrontMinutesPart
            m_deltaToRaceLeaderMSPart = x.m_deltaToRaceLeaderMSPart/1000
            m_deltaToRaceLeaderMinutesPart = x.m_deltaToRaceLeaderMinutesPart/1000
...
def ingest_eventdata(Packet):
    m_header = ingest_Header(Packet.m_header)
    eventStringCode = "".join([chr(x) for x in Packet.m_eventStringCode])
    fastestLapData = Packet.m_eventDetails.m_fastestLap
    retirementData = Packet.m_eventDetails.m_retirement
    teammatePitData = Packet.m_eventDetails.m_teamMateInPits
    winnerData = Packet.m_eventDetails.m_raceWinner
    penaltyData = Packet.m_eventDetails.m_penalty
    speedTrapData = Packet.m_eventDetails.m_speedTrap
    startLightsData = Packet.m_eventDetails.m_startLights
    driveThroughData = Packet.m_eventDetails.m_driveThroughPenaltyServed
    stopGoData = Packet.m_eventDetails.m_stopGoPenaltyServed
    flashbackData = Packet.m_eventDetails.m_flashback
    buttonsData = Packet.m_eventDetails.m_buttons
    overtakeData = Packet.m_eventDetails.m_overtake
    safetyCarData = Packet.m_eventDetails.m_safetyCar
    collisionData = Packet.m_eventDetails.m_collision


    match eventStringCode:
        case "FTLP":
            class fastestLap:
                vehicleIdx = fastestLapData.vehicleIdx
                lapTime = round(fastestLapData.lapTime,3)
            eventDetailData = {"fastestLap":fastestLap}

        case "RTMT":
            class retirement:
                vehicleIdx = retirementData.vehicleIdx
            eventDetailData = {"retirement": retirement}

        case "TMPT":
            class teamMateInPits:
                vehicleIdx = teammatePitData.vehicleIdx
            eventDetailData = {"teamMateInPits": teamMateInPits}

        case "RCWN":
            class raceWinner:
                vehicleIdx = winnerData.vehicleIdx
            eventDetailData = {"raceWinner": raceWinner}
        ...
        case "PENA":
            ...
            eventDetailData = {"penalty": penalty}

        case "SPTP":
            class speedTrap:
                vehicleIdx = speedTrapData.vehicleIdx
            ...
            eventDetailData = {"speedTrap": speedTrap}

        case "STLG":
            class startLights:
                numLights = startLightsData.numLights
            eventDetailData = {"startLights": startLights}

        case "DTSV":
            class driveThroughPenaltyServed:
                vehicleIdx = driveThroughData.vehicleIdx
            eventDetailData = {"driveThroughPenaltyServed": driveThroughPenaltyServed}

        case "SGSV":
            class stopGoPenaltyServed:
                vehicleIdx = stopGoData.vehicleIdx
            eventDetailData = {"stopGoPenaltyServed": stopGoPenaltyServed}

        case "FLBK":
            class flashback:
                flashbackFrameIdentifier = flashbackData.flashbackFrameIdentifier
                flashbackSessionTime = flashbackData.flashbackSessionTime
            eventDetailData = {"flashback": flashback}

        case "BUTN":
            class buttons:
                m_buttonStatus = [i for i in split(buttonsData.m_buttonStatus)]
            eventDetailData = {"buttons": buttons}

        case "OVTK":
            class overtake:
                overtakingVehicleIdx = overtakeData.overtakingVehicleIdx
                beingOvertakenVehicleIdx = overtakeData.beingOvertakenVehicleIdx
            eventDetailData = {"overtake": overtake}

        case "SCAR":
            class safetyCar:
                safetyCarType = safetyCarData.safetyCarType
                eventType = safetyCarData.eventType
            eventDetailData  = {"safetyCar": safetyCar}

        case "COLL":
            class collision:
                vehicle1Idx = collisionData.vehicle1Idx
                vehicle2Idx = collisionData.vehicle2Idx
            eventDetailData = {"collision": collision}
...
def ingest_participantsdata(Packet):
    m_header = ingest_Header(Packet.m_header)
    allParticipantsData=[]
    for x in Packet.m_participants:
        name = "".join([chr(i) for i in x.m_name])

        class participantsData:
            m_aiControlled = x.m_aiControlled
...
def ingest_carsetupdata(Packet):
    m_header=ingest_Header(Packet.m_header)
    allCarSetupData=[]
    for x in Packet.m_car_setups:
        class carSetupData:
            m_frontWing = x.m_frontWing
            ...
            m_rearLeftTyrePressure = round(x.m_rearLeftTyrePressure,5)
            m_rearRightTyrePressure = round(x.m_rearRightTyrePressure,5)
            ...
            m_fuelLoad = round(x.m_fuelLoad,5)
...
def ingest_cartelemetrydata(Packet):
    m_header=ingest_Header(Packet.m_header)
    allCarTelemetryData=[]
...
def ingest_carstatusdata(Packet):
    m_header=ingest_Header(Packet.m_header)
    allCarStatusData=[]
    for x in Packet.m_carStatusData:
        class carStatusData:
            m_tractionControl = x.m_tractionControl
...
def ingest_finalclassificationdata(Packet):
    m_header = ingest_Header(Packet.m_header)
...
def ingest_lobbyinfodata(Packet):
    m_header=ingest_Header(Packet.m_header)
    allLobbyInfoData=[]
...
def ingest_cardamagedata(Packet):
    m_header=ingest_Header(Packet.m_header)
    allCarDamageData=[]
    for x in Packet.m_carDamageData:
        class carDamageData:
            m_tyresWear = expand(x.m_tyresWear)
...
def ingest_tyresetdata(Packet):
    ...
    for x in Packet.m_tyreSetData:
        class tyreSetData:
            ...
            m_lapDeltaTime = round(x.m_lapDeltaTime,5)
            m_fitted = x.m_fitted
...
def ingest_motionexdata(Packet):
    m_header=ingest_Header(Packet.m_header)
