...
def ingest_MotionData(Packet, m_header):
    ...
    data = {"m_header":m_header,
            ...
            #"m_wheelSlip":expand(Packet.m_wheelSlip),
            "m_localVelocityX":round(Packet.m_localVelocityX,5),
            "m_localVelocityY":round(Packet.m_localVelocityY,5),
            "m_localVelocityZ":round(Packet.m_localVelocityZ,5),
            "m_angularVelocityX":round(Packet.m_angularVelocityX,5),
            "m_angularVelocityY":round(Packet.m_angularVelocityY,5),
            "m_angularVelocityZ":round(Packet.m_angularVelocityZ,5),
            "m_angularAccelerationX":round(Packet.m_angularAccelerationX,5),
            "m_angularAccelerationY":round(Packet.m_angularAccelerationY,5),
            "m_angularAccelerationZ":round(Packet.m_angularAccelerationZ,5),
            "m_frontWheelsAngle":round(Packet.m_frontWheelsAngle,5),
            "m_carMotionData":allCarData}

    data.update(expand(Packet.m_suspensionPosition,"m_suspensionPosition"))
    data.update(expand(Packet.m_suspensionVelocity,"m_suspensionVelocity"))
    data.update(expand(Packet.m_suspensionAcceleration,"m_suspensionAcceleration"))
    data.update(expand(Packet.m_wheelSpeed,"m_wheelSpeed"))
    data.update(expand(Packet.m_wheelSlip,"m_wheelSlip"))
    return data
...
def ingest_eventdata(Packet,m_header):
    ...
    if eventStringCode == "FTLP":
        eventDetailData = {"fastestLap":{"vehicleIdx":fastestLapData.vehicleIdx,"lapTime":round(fastestLapData.lapTime,3)}}
        ...
        eventDetailData = {"flashback":{"flashbackFrameIdentifier":flashbackData.flashbackFrameIdentifier,"flashbackSessionTime":flashbackData.flashbackSessionTime}}
...
def ingest_carsetupdata(Packet,m_header):
    ...
    for x in Packet.m_car_setups:
        carSetupData = {"m_frontWing":x.m_frontWing,
                        ...
                        "m_frontLeftTyrePressure":x.m_frontLeftTyrePressure,
                        "m_frontRightTyrePressure":x.m_frontRightTyrePressure,
...
def ingest_cardamagedata(Packet,m_header):
    ...
    for x in Packet.m_carDamageData:
        carDamageData = {"m_frontLeftWingDamage":x.m_frontLeftWingDamage, # hidden
